...
//    Mix_FreeMusic(m_game_state.bgm);
    Mix_FreeChunk(m_game_state.collision_sfx);
    if (shoot_sfx) {
...
void LevelA::set_paused(bool paused) {
    m_is_paused = paused;

    // Optional: Add additional logic when pausing/unpausing
    if (paused) {
        std::cout << "LevelA is now paused." << std::endl;
    } else {
        std::cout << "LevelA is now unpaused." << std::endl;
    }
}

bool LevelA::is_paused() const {
    return m_is_paused;
}
...
//    glm::vec3 jumping_enemy_spawn = find_safe_spawn(m_game_state.map, m_game_state.enemies, ENEMY_COUNT);
    ...
    m_game_state.collision_sfx = Mix_LoadWAV("assets/collision.mp3");
    
    shoot_sfx = Mix_LoadWAV("assets/shoot.wav");
        if (!shoot_sfx) {
...
void LevelA::update(float delta_time) {
    ...
    m_game_state.player->update(delta_time, m_game_state.player, m_game_state.enemies, ENEMY_COUNT, m_game_state.map, m_is_paused);
    ...
    for (int i = 0; i < ENEMY_COUNT - 1; i++) {
        if (m_game_state.enemies[i].is_active()) {
            m_game_state.enemies[i].update(delta_time, &m_game_state.enemies[i], m_game_state.enemies, ENEMY_COUNT, m_game_state.map, m_is_paused);
            ...
            if (m_game_state.player->check_collision(&m_game_state.enemies[i])) {
                ...
                g_lives -= 1;
                Mix_PauseMusic();
                Mix_PlayChannel(-1, m_game_state.collision_sfx, 0);
    ...
    m_game_state.enemies[ENEMY_COUNT - 1].update(delta_time, &m_game_state.enemies[ENEMY_COUNT-1], m_game_state.enemies, 1, m_game_state.map, m_is_paused);
    ...
    m_game_state.enemies[ENEMY_COUNT - 2].update(delta_time, &m_game_state.enemies[ENEMY_COUNT-2], m_game_state.enemies, 1, m_game_state.map, m_is_paused);
    
    // Update and check water drops
    for (int i = 0; i < water_drops.size(); ) {
        water_drops[i]->update(delta_time, water_drops[i], m_game_state.enemies, ENEMY_COUNT, m_game_state.map, m_is_paused);
...
void LevelA::render(ShaderProgram *program) {
    ...
    Utility::draw_text(program, Utility::load_texture("assets/font1.png"), "Lives: " + std::to_string(g_lives), 0.5f, 0.1f, glm::vec3(2.0f, -0.25f, 0.0f));
    
    // Lose
    if (g_lives <= 0) {
        Utility::draw_text(program, Utility::load_texture("assets/font1.png"), "YOU LOSE", 0.9f, 0.15f, glm::vec3(1.8f, -2.5f, 0.0f));
    ...
    // Win
    if (g_level >= 15) {
        Utility::draw_text(program, Utility::load_texture("assets/font1.png"), "YOU WIN", 0.9f, 0.15f, glm::vec3(1.8f, -2.5f, 0.0f));
        SDL_GL_SwapWindow(SDL_GL_GetCurrentWindow());
