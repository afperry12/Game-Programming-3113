...
// Useful enumerations for different types of entities, ai, and ai states
enum EntityType { PLATFORM, PLAYER, ENEMY, AMMO };
enum AIType     { WALKER, GUARD, THROWER, VILLAIN };  // Added VILLAIN type
...
private:
    ...
    int jump_count = 0;  // Track the number of teleports
    float jump_timer = 3.0f;  // Time until the next teleport
...
public:
    ...
    void reset_position() {
        // Reset to starting pos (for recurring enemies)
    ...
    // Overloading to check collisions with map
    void const check_collision_y(Map *map);
    void const check_collision_x(Map *map);
    
    // Update entire entity state
    void update(float delta_time, Entity *player, Entity *collidable_entities, int collidable_entity_count, Map *map, bool is_paused);
    ...
    void handle_jumping(float delta_time, Map* map);
    void handle_villain_teleportation(float delta_time, Entity* player);
    void teleport(Entity *player);
    int get_jump_count() const { return jump_count; }  // Get the number of jumps
    ...
    void const set_original_position(glm::vec3 new_position) { m_original_position = new_position; }
