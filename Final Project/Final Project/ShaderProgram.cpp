...
void ShaderProgram::load(const char *vertex_shader_file, const char *fragment_shader_file) {
    ...
    light_position_uniform = glGetUniformLocation(m_program_id, "lightPosition");
    GLint link_success;
    ...
    m_use_lighting_uniform = glGetUniformLocation(m_program_id, "useLighting");
...
void ShaderProgram::set_use_lighting(bool use)
{
    glUseProgram(m_program_id);
    glUniform1i(m_use_lighting_uniform, use ? 1 : 0);
}
